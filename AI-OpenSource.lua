...
Inline Image Formatting Rules ( UNDER TEST DON'T USE )
 - You can insert custom Roblox images into messages using the following syntax.
  1. Folder image
   ...
   Example:
    • `folder://rbx["Hi"]`
   The NAME must refer to an image name that exists in the provided folderRbx table.
   Do NOT invent image names.
  2. Custom Roblox image
   Use:
    • `file://customrbx("NAME")["rbxassetid://ASSET_ID"]`
   Example:
  ...
  3. Image size (Not working, do not use.)
   An inline image can specify its size using:
    • [size=32]
   Example:
    • Hello `folder://rbx["Pan"]`[size=32] world
    The image becomes approximately 32 × 32 pixels.
    If [size=...] is omitted, use the default text size.
    Do NOT use [size=...] as a replacement for normal text formatting.
    Normal text formatting remains:
    • [size=32]Hello[/size]
      Image size and text size are separate concepts.
  4. Alt text
   An image can have fallback/alternative text:
    • if;Alt("DESCRIPTION")
   Example:
    • `file://customrbx("People")["rbxassetid://123456789"]if;Alt("A People.")`
    Use Alt text when the image cannot be displayed or when a textual description is useful.
  5. Multiple images
   Multiple image tokens are allowed in the same message.
   Example:
    • Here am I! `file://customrbx("Coffee")["rbxassetid://123456789"]` `file://customrbx("MiniFlower")["rbxassetid://123456789"]`
   Do not merge, remove, or rewrite repeated image tokens.
  6. Normal text remains normal text
   Only use image syntax when an image is actually intended.
   Do NOT convert ordinary backtick code into an image.
   For example:
    • `this is normal inline code`
   must remain normal inline code.
  7. Asset safety
   Do not generate or guess Roblox asset IDs.
   If no valid image asset is available, use normal text or Alt text instead.
   Never change an image's asset ID just because another ID looks more appropriate.
  8. Recommended behavior
   When an image is intended to appear inline with text:
    1. Preserve the surrounding text exactly.
    2. Insert the image at the exact location of the image token.
    3. Keep the image from overlapping surrounding text.
    4. Allow multiple images in one message.
    5. Respect [size=NUM] when attached to an image.
    6. Keep normal RichText formatting unchanged.
    7. Do not add emojis or extra explanatory text unless requested.
   Example:
    • Hi `folder://rbx["Pan"]`I'll cook for you!`folder://rbx["Smile"]`.
      should render conceptually as:
       • Hi [IMAGE] I'll cook for you! [IMAGE].

 - Available Image List for "folder://rbx["image"]":
    The following image names are available in "folderRbx": 
     - "Pan"
     - "Flower"
     - "Smile"
     - "Coffee"
     ...
     Example:
      • "folder://rbx["Pan"]"
     Do NOT invent, modify, or guess image names.
...
[color=255,0,0](REMOVED)[/color] **/OpenWebsiteInExperience** | **/OWINE** *URL* - open site
**/Loadstring** *URL* - 'loadstring(game:HttpGet("url")()'
**/Script** *]] .."[[CODE]]".. [[* - run code
[color=255,0,0](REMOVED)[/color] **/Debug** *on/off* - show debug logs (NOT WORKING)
...
[color=255,0,0](REMOVED)[/color] **/SpyChat** *[ON/OFF]* - stream whisper messages (client-side view only) (NOT WORKING)
...
[color=255,0,0](REMOVED)[/color] **/GiveSpaceToCopyButton**~ | **/GSTCB** *[ON/OFF]* - Set TextLabel size to protect text getting overlap by copy button. (Useless now because of Update)
...
function Jimmy.IsPlayerCharacter(model)

	if not model or not model:IsA("Model") then
		return false
	end

	for _, player in ipairs(Jimmy.Services.Players:GetPlayers()) do
...
function Jimmy.HasLineOfSight(target, position)

	if not Jimmy.Hook.Head then
		return false
	end

	local origin = Jimmy.Hook.Head.Position
	...
	return result.Instance
		and result.Instance:IsDescendantOf(target)
end
...
function Jimmy.Scan()
	...
	if not Jimmy.Hook.Active
		or not Jimmy.Hook.Head
	then
		return nil
	end
	...
	local headPosition =
		Jimmy.Hook.Head.Position

	local lookVector =
		Jimmy.Hook.Head.CFrame.LookVector
	...
	for _, object in ipairs(
		...
		if object:IsA("Model")
			and object ~= Jimmy.Hook.Character
			and not Jimmy.IsPlayerCharacter(object)
		then
			...
			then
				...
				if distance > 0
					and distance <= Jimmy.Config.VisionRange
				then
					...
					local halfFOV =
						Jimmy.Config.VisionFOV / 2

					if angle <= halfFOV
						and Jimmy.HasLineOfSight(
							object,
							root.Position
						)
					then
	...
	Jimmy.Vision.Distance =
		best.Distance ~= math.huge
		and best.Distance
		or nil
...
function Jimmy.MoveTo(position, token)

	if not Jimmy.Hook.Humanoid
		or not Jimmy.Hook.Root
	then
		return false
	end

	if Jimmy.Brain.Token ~= token
		or not Jimmy.Hook.Active
	then
		return false
	end

	Jimmy.Movement.Moving = true

	local path =
		Jimmy.Services.Pathfinding:CreatePath({

			AgentRadius =
				Jimmy.Config.AgentRadius,

			AgentHeight =
				Jimmy.Config.AgentHeight,

			AgentCanJump = true,

			WaypointSpacing =
				Jimmy.Config.WaypointSpacing,
	...
	then
		...
		Jimmy.Hook.Humanoid:MoveTo(position)

		Jimmy.Movement.Moving = false
	...
	for _, waypoint in ipairs(
		...
		then
			...
			Jimmy.Movement.Jumping = true
			Jimmy.Hook.Humanoid.Jump = true

		end

		Jimmy.Hook.Humanoid:MoveTo(
			waypoint.Position
		)
		...
		local connection =
			Jimmy.Hook.Humanoid.MoveToFinished:Connect(
				function()
					reached = true
				end
			)
		...
		while Jimmy.Brain.Token == token
			and Jimmy.Hook.Active
			and not reached
			and os.clock() - started
				< Jimmy.Config.MoveTimeout
		do
...
function Jimmy.GetWanderPoint()

	if not Jimmy.Hook.Root then
		return nil
	end

	local look =
		Jimmy.Hook.Root.CFrame.LookVector
	...
	return Jimmy.Hook.Root.Position
		+ flat * math.random(8, 18)
		+ Vector3.new(
			math.random(-6, 6),
			0,
			math.random(-6, 6)
...
function Jimmy.StartBrain()
	...
	Jimmy.Brain.Running = true

	task.spawn(function()

		while Jimmy.Brain.Running
			and Jimmy.Brain.Token == token
			and Jimmy.Hook.Active
			and Jimmy.Hook.Character
			and Jimmy.Hook.Humanoid
			and Jimmy.Hook.Humanoid.Health > 0
		do
			...
			if targetRoot then
				...
				Jimmy.Brain.Target = nil

				Jimmy.Brain.State =
					"Wandering"

				local point =
					Jimmy.GetWanderPoint()
			...
			task.wait(
				Jimmy.Config.BrainInterval
			)

		end

		Jimmy.Brain.Running = false

	end)
end

function Jimmy.StopBrain()

	Jimmy.Brain.Token += 1

	Jimmy.Brain.Running = false
	Jimmy.Brain.State = "Idle"
	Jimmy.Brain.Target = nil

	if Jimmy.Hook.Humanoid then
		Jimmy.Hook.Humanoid:Move(
			Vector3.zero,
			false
		)
	end
...
function Jimmy.HookCharacter(target)

	-- /AllowCam and /Hook are mutually exclusive.
	...
	if Jimmy.Hook.Active then

		return false,
			"Jimmy is already hooked."
	...
	if not target:IsA("Model") then
		...
		return false,
			"Target must be a Model."

	end

	if Jimmy.IsPlayerCharacter(target) then

		return false,
			"Player characters cannot be hooked."
	...
	if not humanoid then
		...
		return false,
			"Model has no Humanoid."

	end

	local root =
		Jimmy.GetRoot(target)

	if not root then

		return false,
			"Model has no usable root part."

	end

	-- Save hook state

	Jimmy.Hook.Character = target
	Jimmy.Hook.Humanoid = humanoid
	Jimmy.Hook.Root = root
	Jimmy.Hook.Head =
		Jimmy.GetHead(target)

	Jimmy.Hook.Active = true

	Jimmy.Chat.Allowed = false
...
function Jimmy.UnhookCharacter()

	if not Jimmy.Hook.Active then
	...
	Jimmy.Hook.Character = nil
	Jimmy.Hook.Humanoid = nil
	Jimmy.Hook.Root = nil
	Jimmy.Hook.Head = nil
	Jimmy.Hook.Active = false

	Jimmy.Chat.Allowed = false

	Jimmy.Vision.Target = nil
	Jimmy.Vision.Distance = nil
	Jimmy.Vision.Angle = nil

	Jimmy.Emote.Current = nil
...
function Jimmy.SetProperty(property, value)

	if not Jimmy.Hook.Humanoid then
	...
	property =
		Jimmy.TrimLower(property)

	if property == "displayname" then

		Jimmy.Hook.Humanoid.DisplayName =
			tostring(value or "")

		Jimmy.Properties.DisplayName =
			Jimmy.Hook.Humanoid.DisplayName
	...
	elseif property == "health" then
		...
		-- NEVER allow Health = 0

		health =
			math.max(
				Jimmy.Config.MinHealth,
				health
			)

		if Jimmy.Hook.Humanoid.MaxHealth > 0 then

			health =
				math.min(
					health,
					Jimmy.Hook.Humanoid.MaxHealth
				)

		end

		Jimmy.Hook.Humanoid.Health =
			health
	...
	elseif property == "sit" then
		...
		value =
			Jimmy.TrimLower(value)

		Jimmy.Hook.Humanoid.Sit =
			value == "on"
			or value == "true"
			or value == "1"
			or value == "yes"
	...
	elseif property == "walkspeed" then
		...
		Jimmy.Hook.Humanoid.WalkSpeed =
			math.clamp(
				speed,
				Jimmy.Config.MinWalkSpeed,
				Jimmy.Config.MaxWalkSpeed
			)
	...
	elseif property == "jumppower" then
		...
		Jimmy.Hook.Humanoid.JumpPower =
			math.clamp(
				power,
				Jimmy.Config.MinJumpPower,
				Jimmy.Config.MaxJumpPower
			)
	...
	elseif property == "platformstand" then
		...
		Jimmy.Hook.Humanoid.PlatformStand =
			value == "on"
			or value == "true"
			or value == "1"
			or value == "yes"
...
function Jimmy.PlayEmote(name)
	...
	name =
		Jimmy.TrimLower(name)

	if not Jimmy.Emote.Allowed[name] then
...
function Jimmy.CheckChatPermission(message)
	...
	for _, phrase in ipairs(
		...
		if text:find(
			...
			Jimmy.Chat.Allowed = true

			if Jimmy.Hook.Head then

				pcall(function()

					Jimmy.Services.Chat:Chat(
						Jimmy.Hook.Head,
						"Alright.",
...
**/AllowSeeChildren** *[ON/OFF]* - ( LAG WARNING, BUG DO NOT USE ) - This is allow an AI to see childrens inside parent while using allowcam.
**/TextStyle** *[INSTANT/EACHTEXT/EACHLINE]* - Text Animation Settings, What kind text styles you'd like?
[color=255,0,0](REMOVED)[/color] **/GiveSpaceToCopyButton**~ | **/GSTCB** *[ON/OFF]* - Set TextLabel size to protect text getting overlap by copy button.
**/TextCounts** *[ALL/USER/AI/SYSTEMONLY]* - Count messages in the chat.
**/InstantScrollDown** - Instantly scroll to the bottom of the chat. Useful when you get lost in older messages.
**/Chat** *TEXT* - Send a message through Roblox Chat.
[color=255,0,0](REMOVED)[/color] **/Hook** *workspace["PATH"]* - Hook a character to give the AI control over it.
